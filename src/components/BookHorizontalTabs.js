...
const BookHorizontalTabs = ({summary, reviews, bio, TOC, add}) => {
    ...
    <Tab className=" uppercase py-2 font-thin tracking-widest  px-3 sm:px-6 text-sm md:text-lg border-b-4 border-white hover:border-black" tabFor="basic-tab-one">Summary</Tab>
        ...
        <div dangerouslySetInnerHTML={{ __html: summary }} className="text-ceci-gray-dark dropCap cms"/>
        </TabPanel>
    ...
    <div className="py-3">
       {reviews[0] ? reviews.map(review => (
            <>
            <blockquote className="pt-3 cms text-ceci-gray-dark" dangerouslySetInnerHTML={{ __html: review.html }}/>
    ...
    <div className="py-2"><p>No review available</p></div>
        ...
        {bio !== "EMPTY: BioNote" ? 
          <div dangerouslySetInnerHTML={{ __html: bio }} className="cms dropCap text-ceci-gray-dark">
            </div> 
          :
