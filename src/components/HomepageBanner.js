   ...
   return (
    ...
    <section className=" max-w-sm md:max-w-4xl  mx-auto py-5  text-ceci-gray-dark">
       ...
       <div id="2" className="col-span-1 flex flex-col justify-center place-content-center pr-2"><Link to={booklink} aria-label={`Featured book: ${book_title}`}>
         <div className=" mx-auto font-sans font-light justify-self-center text-sm md:text-lg tracking-wide text-ceci-gray-mid text-right uppercase">{author}</div>
...
<div className="font-serif py-2 text-lg text-xl max-w-sm text-right text-ceci-gray-mid">{subtitle}</div>
        ...
        <Link to={booklink} aria-label={`Featured book: ${book_title}`}>
         
           {<img src={cover} alt={`Cover of ${book_title}`} width={200} height={375} className="aspect-[67/100] drop-shadow-img homepageimage" loading="eager" /> }
           </Link></div>
          </div>
          ...
          <Link to={booklink} aria-label={`Featured book: ${book_title}`}>{quote}{attr && <div className="self-center text-ceci-gray-mid italic text-lg">- { attr}</div>}</Link></div>
