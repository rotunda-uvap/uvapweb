...
<div className="font-serif py-2 text-lg text-xl max-w-sm text-right text-ceci-gray-mid">{subtitle}</div>
        ...
        <Link to={booklink} aria-label={`Featured book: ${book_title}`}>
          ...
          <div id="4" className="hidden lg:block lg:col-span-1 py-3 font-serif font-light text-2xl self-center text-left md:pl-2">
          <Link to={booklink} aria-label={`Featured book: ${book_title}`}>&ldquo;&nbsp;{quote}&nbsp;&rdquo;{attr && <div className="self-center text-ceci-gray-mid italic text-lg">- { attr}</div>}</Link></div>
