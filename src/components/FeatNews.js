            ...
            <>
              <div className="flex flex-row">
                    ...
                    <div className="py-6 px-5">
                        <div className="flex items-center"><span className="font-light text-gray-800">{data.news.edges[0].node.frontmatter.date}</span>
                        {/* <span className="px-2 py-1 bg-gray-600 text-gray-100 font-bold rounded hover:bg-gray-500">{edge.node.frontmatter.type}</span> */}
                        </div>
                        <div className="m-2"><Link className="text-xl text-gray-700 font-bold hover:underline ml-4" to={`../${ data.news.edges[0].node.frontmatter.type }${ data.news.edges[0].node.fields.slug }`}>{ edge.node.frontmatter.title }</Link>
